`ifndef ALIGNER_VIRTUAL_SEQUENCE_REG_ACCESS_RANDOM_SV
    `define ALIGNER_VIRTUAL_SEQUENCE_REG_ACCESS_RANDOM_SV
    
    class Aligner_virtual_sequence_reg_random extends Aligner_virtual_sequence_base;   
        ...
        `uvm_object_utils(Aligner_virtual_sequence_reg_random)

        function new(string name = "Aligner_virtual_sequence_reg_random");
            super.new(name);
        endfunction
        ...
        virtual task body();
            ...
            for(int unsigned access_idx = 0 ; access_idx < num_accesses; access_idx++) begin
                ...
                case(access)
                    ...
                    default: begin
                        `uvm_fatal("ISSUE", $sformatf("Un-supported value for access: %0s", access.name()))
                    end
                endcase
                wait_random_time();
            end
