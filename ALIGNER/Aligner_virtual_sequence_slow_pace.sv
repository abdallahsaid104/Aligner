`ifndef ALIGNER_VIRTUAL_SEQUENCE_SLOW_PACE_SV 
    `define ALIGNER_VIRTUAL_SEQUENCE_SLOW_PACE_SV

    class Aligner_virtual_sequence_slow_pace extends Aligner_virtual_sequence_base;

        `uvm_object_utils(Aligner_virtual_sequence_slow_pace)

        function new(string name = "Aligner_virtual_sequence_slow_pace");
            super.new(name);
        endfunction
                ...
                begin
                    ...
                    do begin
                        ...
                        `uvm_do_on_with(tx_sequence, p_sequencer.md_tx_sequencer, {
                            num_tx_items == 1                                        -> item.response = MD_OK;
                            num_tx_items > 1 && tx_item_idx < (num_tx_items-1)       -> item.response = MD_OK;
                            num_tx_items > 1 && tx_item_idx == (num_tx_items-1)      -> item.response = MD_ERROR;
                        })
